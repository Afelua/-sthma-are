...
//		echo '<script>alert("Поле пароль не заполненно");</script>'; // То выводим сообщение об ошибке
		...
		if (empty($result['id'])) // Если запрос к бд не возвразяет id пользователя
			...
			{
			$_SESSION['egebio_password'] = $password; // Заносим в сессию  пароль
			$_SESSION['egebio_login'] = $login; // Заносим в сессию  логин
			$_SESSION['egebio_id'] = $result['id']; // Заносим в сессию  id
			$_SESSION['egebio_user_name'] = $result['user_name']; // Заносим в сессию имя пользователя
			$_SESSION['egebio_user_rights'] = $result['user_rights']; // Заносим в сессию права пользователя
		}
	}
}

if (!isset($_SESSION['egebio_login']) || !isset($_SESSION['egebio_id'])){ // если в сессии не загружены логин и id
//вход не произведен
}
if (isset($_SESSION['egebio_login']) && isset($_SESSION['egebio_id'])){ // если в сессии загружены логин и id
//вход произведен
}
if($_SESSION['egebio_user_rights']){define('USER_RIGHTS', $_SESSION['egebio_user_rights']);}
else{define('USER_RIGHTS', '0');}
if (isset($_REQUEST['exit'])) { // если вызвали переменную "exit"
	unset($_SESSION['egebio_password']); // Чистим сессию пароля
	unset($_SESSION['egebio_login']); // Чистим сессию логина
	unset($_SESSION['egebio_id']); // Чистим сессию id
	unset($_SESSION['egebio_user_name']); // Чистим сессию id
	unset($_SESSION['egebio_user_rights']); // Чистим сессию id
